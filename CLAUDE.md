...
|---|---|---|
| `GET /v1/puzzle?site_key=<uuid>` | None | Score request and issue a PoW puzzle (with `algorithm`/`prefix`/`difficulty`) for any non-block tier, or `429` for `Block`. May set `__captcha_trust` cookie. |
