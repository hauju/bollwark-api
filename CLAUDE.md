...
| `POST /v1/sites` | Bearer (`ADMIN_TOKEN`) | Register a site, returns site_key + secret + echoed `allowed_origins`. Optional `allowed_origins` (`http(s)://host[:port]`, max 32) restricts which browser origins `GET /v1/puzzle` serves. Returns 404 when `ADMIN_TOKEN` is unset. |
